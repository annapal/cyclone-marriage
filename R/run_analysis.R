...
# Function to the run the analyses
# Results are saved in "results/" and figures in "figures/"

run_analysis <- function(dat_all) {
  ...
  write_xlsx(res_clean, "results/main.xlsx")
  ...
  res_clean <- broom::tidy(mod, conf.int = TRUE) %>%
    ...
    filter(!is.na(country))
  write_xlsx(res_clean, "results/rural_urban.xlsx")
  ...
  res_clean <- broom::tidy(mod, conf.int = TRUE) %>%
    ...
    select(term, estimate, conf.low, conf.high, std.error, statistic, p.value,
           country, period, year) %>%
    filter(!is.na(country))
  write_xlsx(res_clean, "results/pre_post_2000.xlsx")
  ...
  res_clean <- broom::tidy(mod, conf.int = TRUE) %>%
    ...
    select(term, estimate, conf.low, conf.high, std.error, statistic, p.value, country, year) %>%
    filter(!is.na(country))
  write_xlsx(res_clean, "results/windspeed_64.xlsx")
